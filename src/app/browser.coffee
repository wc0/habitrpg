...
module.exports.app = (app) ->
  ...
  setupGrowlNotifications = (model) ->
    ...
    statsNotification = (html, type) ->
      return if pub.get('stats.lvl') == 0 #don't show notifications if user dead
      growlNotification(html, type)
  ...
  ###
    When model is ready, setup the notification system
  ###
  setupGrowlNotifications(app.model) unless app.model.get('_session.flags.isMobile')
  ...
  app.fn
    browser:
      ...
      # ----------------
      # Expose growl notifications to other scripts
      # ----------------
