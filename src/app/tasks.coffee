...
module.exports.app = (app, model) ->
  ...
  app.fn 'addTask', (e, el) ->
    ...
    newTask.tags = _.reduce user.priv.get('filters'), ((memo,v,k) -> memo[k]=v if v; memo), {}
    ...
    if e.at().path().indexOf(user.id) != -1
  ...
  app.fn 'del', (e) ->
    ...
    user.priv.del "tasks.#{e.get('id')}"
    e.at().remove()


  app.fn 'clearCompleted', (e, el) ->
    completedIds =  _.pluck( _.where(model.get("_page.lists.tasks.#{user.id}.todos"), {completed:true}), 'id')
    todoIds = user.priv.get('ids.todos')

    _.each completedIds, (id) -> user.priv.del "tasks.#{id}"; true
    user.priv.set 'ids.todos', _.difference(todoIds, completedIds)
  ...
  app.fn 'undo', () ->
    ...
    _.each undo.stats, (val, key) -> user.pub.set "stats.#{key}", val; true
    ...
    _.each undo.task, (val, key) ->
      ...
      if key is 'completed'
        user.priv.pass({cron:true}).set("#{taskPath}.completed",val)
      else
        user.priv.set "#{taskPath}.#{key}", val
      true
